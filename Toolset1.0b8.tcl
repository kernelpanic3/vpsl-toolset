...
# Should we show GeeksIRC-specific commands?
    if {$serveraddress == "irc.geeksirc.net:6665"} {
set isgeeksirc 1
...
# Should we show TestIRC-specific commands?
    if {$serveraddress == "127.0.0.1:6669"} {
set istestirc 1
...
# dice
	...
	if {$isgeeksirc == 1 || $isjdnet == 1 || $istestirc == 1} {
	    if {$isMerlionBot == 0} {
...
# GeeksIRC/JDNet-specific command on VirginiaBot/PuppyBot
    if {$isgeeksirc == 1 || $isjdnet == 1} {
if {$isMerlionBot == 0} {
putserv "NOTICE $nick :!lotto\002*\002       - begins a game of Lotto 6-49"
}
}
# GeeksIRC/JDNet/TestIRC-specific command on VirginiaBot/PuppyBot/BlahBot
    if {$isgeeksirc == 1 || $isjdnet == 1 || $istestirc == 1} {
	if {$isMerlionBot == 0} {
...
putlog "No help for $nick on $chan asking about $input"
...
set networkurl "http://fircd.org/"
} elseif {$serveraddress == "127.0.0.1:6669"} {
set network "TestIRC"
set networkurl "http://74.99.75.127/TestIRC/"
} else {
set network "Unknown"
...
set toolsetver "1.0b8"
set toolsetdate "7/19/14"
...
putlog "Kicking and warning $nick - $chan is not a TF2 channel"
...
putlog "$nick: $chan is not a TF2 channel, but udef not set - ignoring"
return 1
}
}
...
putlog "$nick made bad attempt to join $input from $chan (wrong join command), but udef not set - ignoring"
return 1
}
}
...
proc pub_not-join-badcmd-mixnahl {nick uhost hand chan input args} {
    ...
    putlog "Redirecting $nick - bad attempt to join #tf2.mix.nahl from $chan (no join command)"
    ...
    putlog "$nick made bad attempt to join #tf2.mix.nahl from $chan (no join command), but udef not set - ignoring"
return 1
    }
    }
proc pub_not-join-badcmd-mixeuhl {nick uhost hand chan input args} {
    if {[lsearch -exact [channel info $chan] +redirect-notjoin-badcmd] != -1} {
    putlog "Redirecting $nick - bad attempt to join #tf2.mix.euhl from $chan (no join command)"
    putserv "NOTICE $nick :\002\00304ALERT:\002\003 Are you trying to join a TF2 channel?"
    putserv "NOTICE $nick :Try \"/join \#tf2.mix.euhl\" (without quotes)."
    putserv "NOTICE $nick :If you need assistance joining a TF2 channel, please ask a chanop for help. Thanks!"
    } else {
    putlog "$nick made bad attempt to join #tf2.mix.euhl from $chan (no join command), but udef not set - ignoring"
return 1
    }
    }
proc pub_not-join-badcmd-pugnahl {nick uhost hand chan input args} {
    if {[lsearch -exact [channel info $chan] +redirect-notjoin-badcmd] != -1} {
    putlog "Redirecting $nick - bad attempt to join #tf2.pug.nahl from $chan (no join command)"
    putserv "NOTICE $nick :\002\00304ALERT:\002\003 Are you trying to join a TF2 channel?"
    putserv "NOTICE $nick :Try \"/join \#tf2.pug.nahl\" (without quotes)."
    putserv "NOTICE $nick :If you need assistance joining a TF2 channel, please ask a chanop for help. Thanks!"
    } else {
    putlog "$nick made bad attempt to join #tf2.pug.nahl from $chan (no join command), but udef not set - ignoring"
return 1
    }
    }
proc pub_not-join-badcmd-pugna {nick uhost hand chan input args} {
    if {[lsearch -exact [channel info $chan] +redirect-notjoin-badcmd] != -1} {
    putlog "Redirecting $nick - bad attempt to join #tf2.pug.na from $chan (no join command)"
    putserv "NOTICE $nick :\002\00304ALERT:\002\003 Are you trying to join a TF2 channel?"
    putserv "NOTICE $nick :Try \"/join \#tf2.pug.na\" (without quotes)."
    putserv "NOTICE $nick :If you need assistance joining a TF2 channel, please ask a chanop for help. Thanks!"
    } else {
    putlog "$nick made bad attempt to join #tf2.pug.na from $chan (no join command), but udef not set - ignoring"
return 1
    }
    }
proc pub_not-join-badcmd-scrim {nick uhost hand chan input args} {
    if {[lsearch -exact [channel info $chan] +redirect-notjoin-badcmd] != -1} {
    putlog "Redirecting $nick - bad attempt to join #tf2scrim from $chan (no join command)"
    putserv "NOTICE $nick :\002\00304ALERT:\002\003 Are you trying to join a TF2 channel?"
    putserv "NOTICE $nick :Try \"/join \#tf2scrim\" (without quotes)."
    putserv "NOTICE $nick :If you need assistance joining a TF2 channel, please ask a chanop for help. Thanks!"
    } else {
    putlog "$nick made bad attempt to join #tf2scrim from $chan (no join command), but udef not set - ignoring"
return 1
    }
    }
proc pub_not-join-badcmd-mixeu {nick uhost hand chan input args} {
    if {[lsearch -exact [channel info $chan] +redirect-notjoin-badcmd] != -1} {
    putlog "Redirecting $nick - bad attempt to join #tf2.mix.eu from $chan (no join command)"
    putserv "NOTICE $nick :\002\00304ALERT:\002\003 Are you trying to join a TF2 channel?"
    putserv "NOTICE $nick :Try \"/join \#tf2.mix.eu\" (without quotes)."
    putserv "NOTICE $nick :If you need assistance joining a TF2 channel, please ask a chanop for help. Thanks!"
    } else {
    putlog "$nick made bad attempt to join #tf2.mix.eu from $chan (no join command), but udef not set - ignoring"
return 1
    }
    }
proc pub_not-join-badcmd-scrimhl {nick uhost hand chan input args} {
    if {[lsearch -exact [channel info $chan] +redirect-notjoin-badcmd] != -1} {
    putlog "Redirecting $nick - bad attempt to join #tf2scrim.hl from $chan (no join command)"
    putserv "NOTICE $nick :\002\00304ALERT:\002\003 Are you trying to join a TF2 channel?"
    putserv "NOTICE $nick :Try \"/join \#tf2scrim.hl\" (without quotes)."
    putserv "NOTICE $nick :If you need assistance joining a TF2 channel, please ask a chanop for help. Thanks!"
    } else {
    putlog "$nick made bad attempt to join #tf2scrim.hl from $chan (no join command), but udef not set - ignoring"
return 1
    }
    }
...
if {$found == 0 && [eof $fs]} {
putquick "PRIVMSG $chan :critical: line not found"
}
close $fs
}
...
putlog "$nick FOUND THE EASTER EGG IN $chan! :D"
