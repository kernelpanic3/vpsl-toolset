...
#
# Changelog:
# 1.0a3: Reverted commands and binds back to originals. 
# 1.0a2: Fixed all proc/bind overlaps with Double Uno.
# 1.0a1: Split Uno Attack from Double Uno. Baseline coding complete.
# 0.99: Disabled bot play entirely until I can fix it. Message tweaks. Split version numbers.
# 0.98.9mod: Initial revision.
...
set UnoOpFlags		"o|o"
set UnoNTC		"NOTICE"
set UnoCFGFile		"scripts/unoattack.cfg"
set UnoScoreFile 	"UnoAttackScores"
set UnoVersion 		"1.0a3"
...
bind pub - !unosponsors UnoSponsorList
bind pub - !sponsor UnoRandomSponsor
...
bind dcc - unodebugger UnoDebugger
...
proc unoevnt:prerehash {type} {
 unodebug "caught prerehash event"
...
proc unoevnt:prerestart {type} {
 unodebug "caught prerestart event"
 global UnoRobot UnoChan
 UnoStop $UnoRobot "console" $UnoRobot $UnoChan ""
}
...
set UnoWildDrawFourCard "\0031,8\002W\0030,3I\0030,4L\0030,12D\003 \0031,8H\0030,3i\0030,4t\003 \0031,8F\0030,3i\0030,4r\0030,12e\0030,8\003\002 "
set UnoLogo		"\002\0033U\00312N\00313O \00308ATTACK\003!\002"
...
proc UnoStop {nick uhost hand chan txt} {
 ...
 if {(![uno_ischan $chan])||($UnoOn == 0)} {
 unodebug "if failed: uno_ischan is not $chan or UnoOn = 0"
 return
 }
 ...
 # remove player dcc list
 unodebug "remove dcc players"
 uno_removedccplayers

 unodebug "reset variables"
 set UnoOn 0
 set UnoPaused 0
 set UnPlayedRounds 0
 set UnoLastWinner ""
...
proc UnoNext {} {
 ...
 if {!$UnoOn} {
 unodebug "if failed: UnoOn not set"
 return
}
 unodebug "UnoReset"
 UnoReset
 unodebug "setting variables"
 set UnoMode 1
 ...
 unochanmsg "$UnoVersion triggered!"
 unodebug "setting variables"
 set done 0
 while {!$done} {
 ...
 unodebug "checking for existing UnoHand and UnoNickColor"
 if [info exist UnoHand] {
 unodebug "UnoHand found, unsetting"
 unset UnoHand
 }
 if [info exist UnoNickColor] {
 unodebug "UnoNickColor found, unsetting"
 unset UnoNickColor
 }

 unochanmsg "You have \00314\002[UnoDuration $StartGracePeriod]\002\003 to join..."
 unodebug "starting grace period timer"
...
proc UnoBotPlayerJoins {} {
 global UnoPlayers RoundRobin UnoIDX UnoRobot UnoLogo UnoDebug UnoHand UnoNickColor UnoDCCIDX
...
# set UnoDCCIDX($UnoRobot) -1
# unochanmsg "$UnoIDX ... $UnoDCCIDX($UnoRobot)"
#
...
proc UnoStart {} {
 ...
 if {[llength $RoundRobin] == 1} {
  UnoBotPlayerJoins
     set SingularSensation [lindex $RoundRobin 0]
     unomsg "Sorry $SingularSensation, nobody joined! Next game in \00314[UnoDuration $UnoCycleTime]..."
  UnoCycle
  return
 }
 ...
 if {([string range $pcard 0 0] != "W")&&([string range $pcard 1 1] == "H")} {
   uno_adddrawtohand $ThisPlayer $ThisPlayerIDX 2
   unomsg "[unonik $ThisPlayer]\003 \002drew two\002 cards..."
 }

     if {$ThisPlayer != $botnick} {
 unodebug "botnick is not ThisPlayer, showcards"
...
#
# check if player has same color card in hand for whf
#
...
proc UnoDraw {nick uhost hand chan txt} {
 ...
 if {$IsDraw} {
  unontc $nick "You've already used the launcher, $nick, play a card or pass."
  return
 }
 ...
 set LauncherNum [rand 13]

 uno_adddrawtohand $ThisPlayer $ThisPlayerIDX $LauncherNum

 uno_showwhodrew $nick $LauncherNum
...
proc uno_playreversecard {nick pickednum crd} {
 ...
 uno_checkuno $ReversePlayer
    ...
    if {$ThisPlayer == $botnick} {
 unodebug "botnick is ThisPlayer, isdrawreset, toss to UnoBotPlayer"
 uno_isdrawreset
 UnoRobotPlayer
    } else {
 unodebug "botnick is not ThisPlayer, showcards"
 uno_showcards $ThisPlayer $ThisPlayerIDX
 unodebug "isdrawreset"
 uno_isdrawreset
 uno_restartbotplayer
    }
}
...
proc uno_playwilddrawfourcard {nick pickednum crd isrobot} {
 ...
 if {$isrobot} {
  ...
  unodebug "Now triggering showplaywildfour"
     uno_showplaywildfour $nick $PlayerThatDrew $ColorPicker $LauncherNum
  set IsColorChange 1
 } 

 uno_adddrawtohand $PlayerThatDrew $PlayerThatDrewIDX $LauncherNum
...
# number card
#
proc uno_playnumbercard {nick pickednum crd isrobot} {
 global ThisPlayer ThisPlayerIDX RoundRobin botnick
 unodebug "remove from hand, add to discard pile, set NumberCardPlayer"
 uno_removecardfromhand $nick $pickednum

 uno_addtodiscardpile $crd

 set NumberCardPlayer $ThisPlayer
 unodebug "next player"
 uno_nextplayer
 unodebug "if uno_checkwin $NumberCardPlayer"
 if {[uno_checkwin $NumberCardPlayer [uno_cardcolor $crd]]} { return }
 unodebug "showplaycard"
 uno_showplaycard $nick [uno_cardcolor $crd] $ThisPlayer
 unodebug "checkuno"
...
proc uno_playactualcard {nick cardfound pickednum crd isrobot} {
 global CardStats
 unodebug "playactualcard"
 unodebug "isrobot $isrobot"
 unodebug "cardfound $cardfound"
 switch $cardfound {
  0 {
   if {$isrobot} {
    unolog $nick "UnoRobot: oops card"
    unomsg "Oops: UnoRobot oops card (somebody call my owner)"
   } {
    unontc $nick "\002Oops!\002 Not a valid card... draw or play another."
   }
  }
  1 { 
  ...
  6 {
   unodebug "uno_playnumbercard $nick $pickednum $crd"
   uno_playnumbercard $nick $pickednum $crd $isrobot
   unodebug "incr CardStats(played)"
   incr CardStats(played)
   if {!$isrobot} { uno_restartbotplayer }
  }
  7 {
   if {$isrobot} {
    unolog $nick "UnoRobot: oops valid card in-hand"
    unomsg "Oops: UnoRobot: Valid card in-hand (somebody call my owner)"; return
    uno_restartbotplayer
   } {
    unodebug "cardfound $cardfound"
...
proc UnoPlayCard {nick uhost hand chan txt} {
 global UnoMode IsColorChange UnoHand ThisPlayer botnick ThisPlayerIDX
 unodebug "ThisPlayer $ThisPlayer ThisPlayerIDX $ThisPlayerIDX"
 ...
 if {$txt == ""} {return}
 if {$txt == "wh" || $txt == "whf"} {set $txt "wd"}

 set pcard [string toupper [string range $txt 0 1]]
 if {$pcard == "WH" || $pcard == "WHF"} {
 set pcard "WD"
 }
 ...
 while {[lindex $UnoHand($nick) $pcount] != ""} {
  if {$pcard == [lindex $UnoHand($nick) $pcount]} {
   set pcardnum $pcount
      unodebug "pcard $pcard pcount $pcount pcardnum $pcardnum UnoHand for $nick $UnoHand($nick)"
   uno_playactualcard $nick [uno_findcard $nick $pcardnum $pcard] $pcardnum $pcard 0
   return
  }
  incr pcount
 }
 unodebug "pcard $pcard pcount $pcount UnoHand for $nick $UnoHand($nick)"
 ...
 while {$Tier < $TierMax} {
  ...
  while {$CardCount < [llength $UnoHand($ThisPlayer)]} {
   ...
   switch $Tier {
    ...
    7 {if {($hc0 == "W")&&($hc1 == "W")} {return $CardCount}}
   }
   incr CardCount
  }
  incr Tier
...
proc UnoAutoSkip {} {
 ...
 if {[uno_isrobot $ThisPlayerIDX]} {
    unolog "uno" "Oops: Autoskip called while bot players turn"
    unomsg "Oops: Autoskip called while bot players turn (somebody call my owner)"; return}

 if {[uno_timerexists UnoAutoSkip] != ""} {
  unolog "uno" "Oops: Autoskip timer called, but already exists"
  unomsg "Oops: Autoskip timer called, but already exists (somebody call my owner)"
  return
 }
 ...
 if {!$InChannel || ($Idler == $UnoLastIdler)} {
  if {!$InChannel} {
   unomsg "[unonik $Idler]\003 left the channel and is removed from the game."
  } {
   unomsg "[unonik $Idler]\003 has been idle twice in a row and is removed from the game."
   set UnoLastIdler ""
  }
  if {$IsColorChange == 1} {
   if {$Idler == $ColorPicker} {
    ...
    unomsg "Oops: UnoAutoRemove color change set but $Idler not color picker (somebody call my owner)"
   }
  }
...
proc UnoPause {nick uhost hand chan txt} {
 ...
 unodebug "UnoPause routine (Attack)"
    if {![uno_isrunning $chan]} {
unodebug "uno_isrunning not met, return"
return
}
 unodebug "if validuser"
 if {([validuser $nick])&&([matchattr $nick $UnoOpFlags $UnoChan])} {
  if {!$UnoPaused} {
   unodebug "UnoPaused set"
   set UnoPaused 1
   unodebug "UnoUnbindCmds run"
   UnoUnbindCmds
   unochanmsg "\00304 paused \003by $nick..."
  } {
   unodebug "UnoPaused set"
   set UnoPaused 0
   unodebug "UnoBindCmds run"
   UnoBindCmds
   unodebug "uno_autoskipreset run"
   uno_autoskipreset $nick
   unochanmsg "\00303 resumed \003by $nick..."
  }
 } else {unodebug "validuser not met with $nick"}
}
...
proc UnoRemove {nick uhost hand chan txt} {
 ...
 if {$UnoOpRemove > 0} {
  unomsg "[unonik $nick]\003 was removed from the game by $UnoOpNick..."
 } {
  unontc $nick "You are now removed from the current game of $UnoLogo..."
  unomsg "[unonik $nick]\003 left the game..."
 }
 ...
 if {$IsColorChange == 1} {
  if {$nick == $ColorPicker} {
   # Make A Color Choice
   set cip [uno_pickcolor]
   unomsg "[unonik $nick]\003 was choosing a color...randomly selected $cip."
   set IsColorChange 0
  } {
...
proc uno_restartbotplayer {} {
 global UnoMode ThisPlayerIDX RobotRestartPeriod UnoBotTimer ThisPlayer
 unodebug "$UnoMode $ThisPlayer $ThisPlayerIDX $RobotRestartPeriod $UnoBotTimer"
 if {$UnoMode != 2} {return}
 if {![uno_isrobot $ThisPlayerIDX]} {return}
 unodebug "set UnoBotTimer to RobotRestartPeriod"
...
proc UnoCmds {nick uhost hand chan txt} {
 global UnoLogo UnoVersion
 if {![uno_ischan $chan]} {return}
 unomsg "$UnoLogo Commands: !unoattack !stop !remove \[nick\] !unowon \[nick\] !unocmds"
 unomsg "$UnoLogo Stats: !unotop10 \[games\|wins\|21\] !unotop3last !unostats !unorecords"
 unomsg "$UnoLogo Chan Commands: ca=cards cd=card tu=turn od=order ct=count st=stats ti=time"
 unomsg "$UnoLogo $UnoVersion"
 unomsg "$UnoLogo Have fun!"
 return
}
...
proc UnoVersion {nick uhost hand chan txt} {
 global UnoVersion
 unochanmsg "$UnoVersion by VASirens. Original Color Uno by Marky.\003"
 return
}
...
proc UnoShowCards {nick uhost hand chan txt} {
 global UnoHand ThisPlayerIDX UnoRobot ThisPlayer
 ...
 if {![uno_isrobot $ThisPlayerIDX] || $ThisPlayer != $UnoRobot} { unontc $nick "$Card\003" }
...
# sponsor ads
# sponsor 1 - mrboojay productions
proc UnoSponsor1 {} {
 unochanmsg "is sponsored by \0032\002mrboojay \0030pro\0034ductions\0031\002 - makers of \002Delhi Thunderbeam\002 motors."
 return
 }
# sponsor 2 - Mann Consolidated
proc UnoSponsor2 {} {
 unochanmsg "is sponsored by \0035\002Mann Consolidated\0031\002 - \002making machines simple\002."
 return
 }
# sponsor 3 - Nathan Diniz/siren_face2000
proc UnoSponsor3 {} {
 unochanmsg "is sponsored by \0035\002Nathan Diniz\0031\002 - \002riding nerdy for over 30 years\002. Online at \002http://hp41.org\002."
 return
 }
# sponsor 4 - none ATM
proc UnoSponsor4 {} {
 unochanmsg "\0035\002Sponsor this game\0031\002 - \002contact VASirens for info\002."
 return
 }
# sponsor 5 - none ATM
proc UnoSponsor5 {} {
...
# list sponsors
proc UnoSponsorList {nick uhost hand chan txt} {
 unochanmsg "sponsors: \0025\002 berths displayed below."
 unochanmsg "Sponsor \0021\002\: \"\0032\002mrboojay \0030pro\0034ductions\0031\002 - makers of \002Delhi Thunderbeam\002 motors\""
 unochanmsg "Sponsor \0022\002: \"\0035\002Mann Consolidated\0031\002 - \002making machines simple\002\""
 unochanmsg "Sponsor \0023\002: \"\0035\002Nathan Diniz\0031\002 - \002riding nerdy for over 30 years\002. Online at \002http://hp41.org\002\""
 unochanmsg "Sponsor \0024\002: empty"
 unochanmsg "Sponsor \0025\002: empty"
 unochanmsg "For sponsorship inquiries, contact \002VASirens\002."
 return
 }

# display random sponsor
proc UnoRandomSponsor {nick uhost hand chan txt} {
 global UnoSponsorNumber

 switch $UnoSponsorNumber {
  1 UnoSponsor1
  2 UnoSponsor2
  3 UnoSponsor3
  4 UnoSponsor4
 ...
 incr UnoSponsorNumber

 if {$UnoSponsorNumber > 5} {set UnoSponsorNumber 0}
...
proc UnoScoreAdvertise {} {
 ...
 switch $UnoAdNumber {
  0 {UnoTop10 1}
  1 {UnoSponsor1 0}
  2 {UnoLastMonthTop3 $UnoRobot none none $UnoChan 0}
  3 {UnoSponsor2 0}
  4 {UnoTop10 0}
  5 {UnoSponsor3 0}
  6 {UnoRecords $UnoRobot none none $UnoChan ""}
  7 {UnoSponsor4 0}
  8 {UnoTop10 2}
  9 {UnoSponsor5 0}
  10 {UnoPlayed $UnoRobot none none $UnoChan ""}
  11 {UnoHighScore $UnoRobot none none $UnoChan ""}
  12 {UnoTopFast $UnoRobot none none $UnoChan ""}
 }

 incr UnoAdNumber

 if {$UnoAdNumber > 12} {set UnoAdNumber 0}
...
# played wild hit fire
proc uno_showplaywildfour {who skipper chooser firednum} {
 global UnoWildDrawFourCard UnoNickColor
    unodebug "This is showplaywildfour"
    unodebug "firednum is $firednum"
    if {$firednum == 1} {
    unomsg "[unonik $who]\003 plays $UnoWildDrawFourCard[unonik $skipper]\003 presses launcher button, draws \002$firednum\002 card, and is skipped...choose a color [unonik $chooser]!"
    } else {
    unomsg "[unonik $who]\003 plays $UnoWildDrawFourCard[unonik $skipper]\003 presses launcher button, draws \002$firednum\002 cards, and is skipped...choose a color [unonik $chooser]!"
    }
}
...
# who drew a card
proc uno_showwhodrew {who LauncherNum} {
 unomsg "[unonik $who]\003 \002presses launcher button\002..."
    if {$LauncherNum == 1} {
 unomsg "[unonik $who]\003 \002draws\002 $LauncherNum card..."
    } else {
      unomsg "[unonik $who]\003 \002draws\002 $LauncherNum cards..."
    }
}
...
proc uno_showdraw {idx crd} {
 ...
 unontc [lindex $UnoIDX $idx] "You drew: $crd"
}
...
proc uno_showwindefault {who} {
 ...
 unomsg "[unonik $who] \002\00309W\00312i\00313n\00308s\002 $UnoLogo by default."
 set UnoWinDefault 1
}
...
proc unodebug {what} {
global UnoChan UnoDebugger
if {$UnoDebugger == 1} {
putquick "PRIVMSG $UnoChan :Debug: $what"
} else {return}
}
...
# debugger
proc UnoDebugger {nick idx input} {
global UnoOn UnoChan UnoDebugger
    # if {$UnoOn == 1} {
#	unogntc $nick "Oops: Debugger cannot be enabled/disabled with game in progress"
#return
#    }
    if {$UnoDebugger == 0} {
set UnoDebugger 1
putlog "Uno Attack: Debugger enabled by $nick"
unochanmsg "debugger enabled!"
unomsg "\002WARNING:\002 Other hands \002\0034,00may be revealed\003\002 with the debugger on."
unomsg "Do not use during actual games!"
    } elseif {$UnoDebugger == 1} {
set UnoDebugger 0
putlog "Uno Attack: Debugger disabled by $nick"
unochanmsg "debugger disabled!"
    }
}

putlog "Loaded Double Color Uno $UnoVersion"
putlog "Copyright (C) 2004-2011 by Marky"
putlog "Copyright (C) 2014 VASirens"
