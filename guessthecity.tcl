...
set tgqdb "scripts/guessthecity.txt"
...
scripts/trivragen/guessthecity.txt&guessthecity
...
set tgscf "scripts/guessthecity.scores"
...
set tgerrfil "scripts/guessthecity.errors"

#  The e-mail address to send error reports to.
set tgerremail "connoisseur5299@gmail.com"
...
set tghtmlfile "/usr/bitnami/apache2/htdocs/guessthecity.html"
...
set tgcongrats [list "Congratulations" "Well done" "Nice going" "Way to go" "You got it" "That's the way" "Awesome" "Great job"]
...
if {![info exists tgplaying]} {
	set ctcp-version "${ctcp-version} Triviaplus.tcl M.C.-questions and categories. Based upon $tgver ($tgrel) from www.eggdrop.za.net)"
	set tgplaying 0
}
...
bind pubm $tgflagschoose "$tgchan %$tgchoosecmd" give_choicenew
proc give_choicenew {nick uhost handle channel args} {
 global questions tgplaying tgqdb quest usedq questionno questionfiles gameplaying gameonoff chan tgquestionstotal tgtotalquestfiles tgquestionfiles tgset_catgentrig tgchoosecmd 
  if {$tgplaying != 0} {
  putserv "PRIVMSG $channel :\002Oops!\002 The game is already running."
	return
}
	set questionno 0
	set quest ""
...
set tgtotalquestfiles [llength $questionfiles]
 		set i 0
			...
			if {$tgqdb ==$tgfilename} {tggamemsg "[tgcolqbody] Current category:\00304$tgdesc[tgcolqbody] with \00304$tgquestionstotal [tgcolqbody]questions"}
			incr i
			}
			...
			close $qfile
			incr i
			tggamemsg "\00304$tgdesc[tgcolqbody] with \00304$tgquestioncount\003 [tgcolqbody]questions."
				bind pub -|- $tgdesc set_catgen
				lappend tgset_catgentrig $tgdesc
			}
			tggamemsg "[tgcolqbody] Which category do you want to use?"
}

#set category
proc set_catgen {nick uhost handle channel text} {
 global tgqdb tgtotalquestfiles tgquestionfiles tgdesc lastbind tgset_catgentrig tgcmdstart
 			set i 0
 			while {$i<$tgtotalquestfiles} {
			set tgfilename [lindex [split [lindex $tgquestionfiles $i] &] 0]
			set tgdesc [lindex [split [lindex $tgquestionfiles $i] &] 1]
			if {[strlwr $tgdesc] != [tgstripcodes [strlwr [string trim $lastbind]]]} {
			incr i
			}
			if {[strlwr $tgdesc] == [tgstripcodes [strlwr [string trim $lastbind]]]} {
			...
			set tgqdb $tgfilename
						set i 0
			 			while {$i<$tgtotalquestfiles} {
			 			unbind pub -|- [lindex $tgset_catgentrig $i] set_catgen		
			 			incr i
			 			}
			return
			}	
...
proc tgstart {nick host hand chan text} {
	...
	if {[strlwr $tgchan]==[strlwr $chan]} {
		...
		if {$tgcheat != "ON"} {set tgcheat "OFF"}			
			tggamemsg "[tgcolstart]\002WELCOME TO SIREN TRIVIA!\002 For another category use: $tgchoosecmd [tgcolstart]"
			tgnextq
			set tgplaying 1
			set tgstreak 0
			set tgmissed 0
...
proc tgstop {nick host hand chan text} {
	...
	if {[strlwr $tgchan]==[strlwr $chan]} {
		if {$tgplaying==1} {
			tggamemsg "[tgcolstop]Siren Trivia has been stopped by $nick."
...
proc tgerrremind {} {
	...
	tggamemsg "[tgcolerr]\002Remember:\002 to report errors in questions or answers, type: /msg $botnick $tgcmderror <number> \[description\]"
	set tgerrremindtimer [timer $tgerrremindtime tgerrremind]
}
...
proc tghint {} {
	...
	if {$tghintnum >= [expr $tgmaxhintcurrent+1]} {
		...
		if {$tgmaxmissed>0&&$tgmissed>=$tgmaxmissed} {
			append _msg " That's $tgmissed questions gone unanswered! Siren Trivia has been stopped. To start the game again, type $tgcmdstart."
			tgquietstop
		} else {
...
proc tgshowtop10 {nick host hand chan text} {
	...
	if {[strlwr $chan]==[strlwr $tgchan]} {
		...
		if {$tgscorestotal>0} {
			...
			tggamemsg "[tgcolmisc1]\002Hmm.\002 The score table is empty."
		}
	}
}
...
#cheat protection on
	   ...
	   if {$firstone !=-1} {tggamemsg "[tgcolstart] You already gave a wrong answer $nick. Wait for the hint or the next question."}		
		}
	}
 	...
 	if {[strlwr $tgcurrentanswer] == [tgstripcodes [strlwr [string trim $text]]]} {
		...
		if {$firstone !=-1} {tggamemsg "[tgcolstart]\002Duh.\002 Do you want the bot to ignore you, $nick?"}
		}
		return
	}
...
proc tgerror {nick host hand text} {
	...
	close $_file
	if {$tgerrmethod==1} {
		exec mail -s "Siren Trivia error report from $nick" $tgerremail < $_fname
		file delete $_fname
		[tgpriv] $nick "[tgbold][tgcolmisc1]Thanks! Your error report has been emailed to VASirens."
	} else {
...
proc tghtml {} {
	...
	puts $_file "<html>"
	puts $_file " <head>"
	puts $_file "  <title>Siren Trivia with $botnick: $tgchan on [lindex [split $server :] 0]</title>"
	...
	if {![onchan $botnick $tgchan]} {
		puts $_file "  <p>Hmmm... for some reason $botnick is not on $tgchan at the moment. Please try again later.</p>"
	} else {
		puts $_file "  <p>Siren Trivia is currently <b>[expr $tgplaying==1?"on":"off"]</b>. There are <b>$tgquestionstotal</b> questions in the database."
